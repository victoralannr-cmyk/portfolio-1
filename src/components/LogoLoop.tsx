...
export interface LogoItemSource {
  src: string;
  srcSet?: string;
  sizes?: string;
  width?: number;
...
export type LogoItem = LogoItemSource | LogoItemNode;
...
export const LogoLoop: React.FC<LogoLoopProps> = memo(
  ({
    ...
    // Robust dimension polling fallback
    useEffect(() => {
      const activeSize = isVertical ? seqHeight : seqWidth;
      if (activeSize === 0) {
    ...
    }, [seqWidth, seqHeight, isVertical, updateDimensions]);
